...
# Take photo button
        take_photo_btn = Button(btn_frame1, text="Take Photo Sample", width=33, font=(
            "times new roman", 12, "bold"), bg="blue", fg="white")
...
# *********************Function Declaration*******************
    ...
    def add_data(self):
        if self.var_dep.get()=="Select Department" or self.var_std_name.get()=="" or self.var_std_id=="":
            ...
            try:
                conn=mysql.connector.connect(host="localhost",username="root",port="3306",password="kishan8283#",database="face_recognizer")
...
#************** Update function **********
    ...
    def Update_data(self):
        if self.var_dep.get()=="Select Department" or self.var_std_name.get()=="" or self.var_std_id=="":
            ...
            try:
                Upadate=messagebox.askyesno("Update","Do you want to update this detail",parent=self.root)
                if Upadate>0:
                    conn=mysql.connector.connect(host="localhost",username="root",port="3306",password="kishan8283#",database="face_recognizer")
                    my_cursor=conn.cursor()
